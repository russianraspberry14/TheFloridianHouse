...
# Plotting
plt.plot(df['month_year'], df['price_reduced_count'], color='g', label='Price Reduction Count')
plt.plot(filtered_df['month_year'], filtered_df['incident_value'], marker="*", color='r', label='Disasters')
plt.xlabel("Month and Year")
plt.ylabel("Price Reduction Count")
plt.title('Price Reduction Count and Disasters vs Month')
plt.legend()
...
for line in plt.gca().get_lines():
    y = line.get_ydata()
    x = line.get_xdata()
    mode = 'lines' if line.get_label() == 'Price Reduction Count' else 'markers'
    name = line.get_label()
