...
# Plotting
plt.plot(df['month_year'], df['total_listing_count'], color='g', label='Total Listings')
plt.plot(filtered_df['month_year'], filtered_df['incident_value'], marker="*", color='r', label='Disasters')
plt.xlabel("Month and Year")
plt.ylabel("Total Listings")
plt.title('Total Listings and Disasters vs Month')
plt.legend()
...
for line in plt.gca().get_lines():
    y = line.get_ydata()
    x = line.get_xdata()
    mode = 'lines' if line.get_label() == 'Total Listings' else 'markers'
    name = line.get_label()
