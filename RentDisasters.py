...
# Plotting
plt.plot(df['month_year'], df['rent (average)'], color='g', label='Average Rent')
plt.plot(filtered_df['month_year'], filtered_df['incident_value'], marker="*", color='r', label='Disasters')
plt.xlabel("Month and Year")
plt.ylabel("Average Rent")
plt.title('Average Rent and Disasters vs Month')
plt.legend()
...
for line in plt.gca().get_lines():
    y = line.get_ydata()
    x = line.get_xdata()
    mode = 'lines' if line.get_label() == 'Average Rent' else 'markers'
    name = line.get_label()
