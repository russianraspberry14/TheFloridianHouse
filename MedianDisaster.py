...
# Plotting
plt.plot(df['month_year'], df['median_listing_price'], color='g', label='Median Price')
plt.plot(filtered_df['month_year'], filtered_df['incident_value'], marker="*", color='r', label='Disasters')
plt.xlabel("Month and Year")
plt.ylabel("Median Price")
plt.title('Median Price and Disasters vs Month')
plt.legend()
...
for line in plt.gca().get_lines():
    y = line.get_ydata()
    x = line.get_xdata()
    mode = 'lines' if line.get_label() == 'Median Price' else 'markers'
    name = line.get_label()
